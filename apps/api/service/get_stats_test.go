...
func TestGetStats(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			resp, err := svc.GetStats(context.Background(), tc.code)
			...
			if resp.ClickCount != 9 {
				t.Fatalf("expected click_count %d, got %d", 9, resp.ClickCount)
			}
			if resp.CreatedAt != "2025-01-01T00:00:00Z" {
				t.Fatalf("expected created_at %q, got %q", "2025-01-01T00:00:00Z", resp.CreatedAt)
			}
		})
	}
}
